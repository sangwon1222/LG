...
export class ThumbGroup extends PIXI.Sprite {
  ...
  registEvent() {
    this.interactive = true;
    this.buttonMode = true;
    this.hitArea = new PIXI.Rectangle(0, 450, 1280 * 3, 400);
...
export class Popup extends PIXI.Container {
  ...
  private mClosePop: PIXI.Graphics;
  ...
  async onInit() {
    ...
    this.addChild(this.mBG, this.mHeader);
    this.registBtnEvent();
  }
  ...
  createBtn(): Promise<void> {
    return new Promise<void>((resolve) => {
      ...
      this.mClosePop = new PIXI.Graphics();
      this.mClosePop.beginFill(0xff0000, 0.1);
      this.mClosePop.drawRect(0, 0, 1000, 360);
      this.mClosePop.endFill();
      this.mClosePop.alpha = 0;
      this.mClosePop.position.set(140, 112);
      this.addChild(this.mClosePop);
