...
export class Page11 extends SceneBase {
  ...
  private mQuestSpeakerAry: Array<PIXI.Sprite>;
  ...
  async onInit() {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      tQuestSndAry[i] = tQuestSnd;
      tQuestSpeaker.on("pointertap", () => {
        pixiSound.stopAll();
        tQuestSndAry[i].play();
      });

      this.mQuestSpeakerAry[i] = tQuestSpeaker;
    ...
    for (let i = 0; i < 4; i++) {
      ...
      tAnswerBoxSp.onPointerUp = (evt: PIXI.InteractionEvent) => {
        ...
        if (tHitAreaAry[i].hitArea.contains(x, y)) {
          ...
          this.mClearCnt--;
          if (this.mClearCnt < 0) {
            this.clearAni(tHitAreaAry, tQuestSndAry, tCorrectIdxAry);
          } else {
  ...
  private async clearAni(
    tSpAry: Array<any>,
    tSndAry: Array<any>,
    tRowAry: Array<number>
  ) {
    // let tClear = true;
    for (let i = 0; i < 4; i++) {
      this.mQuestSpeakerAry[i].interactive = false;
      this.mQuestSpeakerAry[i].buttonMode = false;
    }

    await this.tweenMotion(1);
    this.hideFingerGuide();
    for (let i = 0; i < 4; i++) {
      const tIdx = tRowAry[i];
      gsap.to(tSpAry[tIdx].scale, { x: 1.2, y: 1.2, duration: 0.5 });
      tSndAry[i].play();

      await this.tweenMotion(tSndAry[i].duration);
      gsap.to(tSpAry[tIdx].scale, { x: 1, y: 1, duration: 0.5 });
